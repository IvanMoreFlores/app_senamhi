...
@Component({
  ...
  templateUrl: './hidrologia.page.html',
  styleUrls: ['./hidrologia.page.scss'],
})
export class HidrologiaPage implements OnInit, OnDestroy {
      ...
      buttons: [
        {
          ...
          handler: () => {
            ...
            // tslint:disable-next-line: max-line-length
            this.router.navigate(['/medicion', this.activatedRoute.snapshot.paramMap.get('id'), ID_HORA_SINOPTICA, 'H']);
          },
        },
      ],
    });
  ...
  PageMeteoro() {
    this.backButtonSubscription.unsubscribe();
    this.router.navigate(['/aforo', this.activatedRoute.snapshot.paramMap.get('id')]);
  }
  ...
  async asyncLoading() {
    ...
    array.forEach(element => {
      console.log(element);
      this.sqlite.deleteTable(element).then(() => {
    ...
    this.backButtonSubscription = this.platform.backButton.subscribeWithPriority(0, () => {
      if (!this.div_horarios && this.div_opciones) {
        if (this.routerOutlet && this.routerOutlet.canGoBack()) {
          ...
          // tslint:disable-next-line: max-line-length
        } else if (this.router.url === '/hidrologia/' + this.activatedRoute.snapshot.paramMap.get('id') + '/' + this.activatedRoute.snapshot.paramMap.get('tipo')) {
  ...
  ngOnDestroy() {
    console.log('ngOnDestroy en hodrologia');
    this.backButtonSubscription.unsubscribe();
  }
